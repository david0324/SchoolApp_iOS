...
- (void)reloadData
{
    ...
    int sessionWidth = self.view.frame.size.width ;
    for (int i = 0; i < session; i ++) {
        PHSessionView * sessionView = sessionViews[@(i)];
        if (!sessionView) {
            sessionView = [[PHSessionView alloc] initWithFrame:CGRectMake(0, 0, sessionWidth, sessionHeight)];
...
//            [sessionView.button setTitle:sesionTitle forState:UIControlStateNormal];
    ...
    for (int i = 0; i < session; i ++) {
        ...
        for (int j = 0; j < [rowsOfSession[i] intValue]; j ++) {
            ...
            UIButton *btn_partystories = [[UIButton alloc] init];
            btn_partystories.frame = CGRectMake(73, 97, 129, 30);
            [btn_partystories setTitle:@"Party Stories" forState:UIControlStateNormal];
            [btn_partystories addTarget:self action:@selector(rowDidTouch:) forControlEvents:UIControlEventTouchUpInside];
            btn_partystories.titleLabel.font = [UIFont fontWithName:@"Helvetica" size:22];
            btn_partystories.tag = 0;
            [sessionView.containView addSubview:btn_partystories];

            UIButton *btn_entourage = [[UIButton alloc] init];
            btn_entourage.frame = CGRectMake(70, 150, 129, 30);
            [btn_entourage setTitle:@"Entourage" forState:UIControlStateNormal];
            [btn_entourage addTarget:self action:@selector(rowDidTouch:) forControlEvents:UIControlEventTouchUpInside];
            btn_entourage.titleLabel.font = [UIFont fontWithName:@"Helvetica" size:22];
            btn_entourage.tag = 1;
            [sessionView.containView addSubview:btn_entourage];

            
            UIButton *btn_remember = [[UIButton alloc] init];
            btn_remember.frame = CGRectMake(73, 216, 129, 30);
            [btn_remember setTitle:@"Remember?" forState:UIControlStateNormal];
            [btn_remember addTarget:self action:@selector(rowDidTouch:) forControlEvents:UIControlEventTouchUpInside];
            btn_remember.titleLabel.font = [UIFont fontWithName:@"Helvetica" size:22];
            btn_remember.tag = 2;
            [sessionView.containView addSubview:btn_remember];

            UIButton *btn_settings = [[UIButton alloc] init];
            btn_settings.frame = CGRectMake(73, 276, 129, 30);
            [btn_settings setTitle:@"Settings" forState:UIControlStateNormal];
            [btn_settings addTarget:self action:@selector(rowDidTouch:) forControlEvents:UIControlEventTouchUpInside];
            btn_settings.titleLabel.font = [UIFont fontWithName:@"Helvetica" size:22];
            btn_settings.tag = 3;
            [sessionView.containView addSubview:btn_settings];

            UIButton *btn_logout = [[UIButton alloc] init];
            btn_logout.frame = CGRectMake(32, 427, 170, 33);
            [btn_logout setBackgroundImage:[UIImage imageNamed:@"btn_logout.png"] forState:UIControlStateNormal];
            [btn_logout addTarget:self action:@selector(rowDidTouchLogout:) forControlEvents:UIControlEventTouchUpInside];
            [sessionView.containView addSubview:btn_logout];
...
- (void)rowDidTouch:(UIButton*)button
{
    ...
    NSLog(@"superview.tag = %ld",(long)button.superview.tag);
    
    NSLog(@"self.currentIndexPath = %@", self.currentIndexPath);
    // Should select ?
    ...
    NSLog(@"[self.delegate respondsToSelector:@selector(shouldSelectRowAtIndex:)] = %d",[self.delegate respondsToSelector:@selector(shouldSelectRowAtIndex:)]);
...
- (void)rowDidTouchLogout:(UIButton*)button
{
